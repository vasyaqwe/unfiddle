...
export const panZoomAtom = atom(
   null,
   (
      _get,
      set,
      action: { x: number; y: number; behavior?: "smooth" | "auto" },
   ) => {
      set(_zoomAtom, (prev) => {
         ...
         if (d3Container && d3Zoom) {
            ...
            const shouldPanX =
               viewportX < viewportPadding ||
               viewportX > viewport.width - viewportPadding
            const shouldPanY =
               viewportY < viewportPadding ||
               viewportY > viewport.height - viewportPadding
            ...
            if (shouldPanX || shouldPanY) {
               ...
               d3Container
                  .transition()
                  .duration(
                     action.behavior === "auto" ? 0 : TRANSITION_DURATION_MS,
                  )
...
export const changeZoomAtom = atom(
   ...
   (
      _get,
      set,
      action: "zoom-50%" | "zoom-100%" | "zoom-fit" | "zoom-in" | "zoom-out",
   ) => {
      set(_zoomAtom, (prev) => {
