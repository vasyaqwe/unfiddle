...
import { CreateEstimateMessage } from "@/estimate/message/components/create-estimate-message"
import { EstimateMessage } from "@/estimate/message/components/estimate-message"
import {
   useInitialScrollToBottom,
   useOnMessageDelete,
   useOnMessageInsert,
} from "@/estimate/message/hooks"
import { useEstimateMessagesQuery } from "@/estimate/message/queries"
import { useMarkMessagesAsRead } from "@/estimate/message/read/mutations"
import {
   Header,
   HeaderBackButton,
   HeaderSeparator,
...
function RouteComponent() {
   ...
   const query = useEstimateMessagesQuery(estimate.id)
   const [unreadCount, setUnreadCount] = React.useState(0)
   const editingMessageId = useAtomValue(editingMessageIdAtom)
   const tabFocused = useTabFocused()
   const readMessages = useMarkMessagesAsRead(estimate.id)
      ...
      <>
         <Header className="md:flex md:px-1.75">
            <HeaderBackButton
               ...
               onClick={() =>
                  navigate({ to: "/$workspaceId/estimate/$estimateId", params })
               }
            />
            <HeaderTitle>
